...
const screenForNames = (arr) => {
  const namePattern = /^(Mr\.|Mrs\.|Ms\.|Dr\.)\s+[A-Za-z\s]+$/;

  // Use filter to keep only matching strings
...
const toTitleCase = (arr) => {
  ...
  for (const str of arr) {
    if (str.length > 0) {
      const titleCaseStr = str.charAt(0).toUpperCase() + str.slice(1);
...
let biggerThanLuke = (arr) => {
  ...
  const charactersWithGreaterMass = arr.filter(
    (character) => parseFloat(character.mass) > lukeMass
  );

  const characterNames = charactersWithGreaterMass.map(
