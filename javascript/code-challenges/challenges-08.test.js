...
const sortByChildren = (charArray) => {
  charArray.sort((a, b) => {
    if (a.children.length !== b.children.length) {
      ...
      const houseNameA = a.house.toLowerCase();
      const houseNameB = b.house.toLowerCase();

      const regex = /^[a-zA-Z]+/;
      const matchA = houseNameA.match(regex)[0];
      const matchB = houseNameB.match(regex)[0];

      for (let i = 0; i < Math.min(matchA.length, matchB.length); i++) {
        if (matchA[i] !== matchB[i]) {
          return matchA.charCodeAt(i) - matchB.charCodeAt(i);
        }
      }
